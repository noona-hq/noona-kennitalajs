class Kennitala {
  constructor(kennitala) {
    ...
    if (validate.valid) {
      ...
      if (age.years < 0) {
        return { valid: false, reason: 'Person is not born', errorCode: 6 };
      }
  ...
  isValid(kt) {
    ...
    if (!(dd > 0 && dd < 72) || (dd > 31 && dd < 41))
      return { valid: false, reason: 'Birthdate is out of range (digits 1 and 2)', errorCode: 2 };
    ...
    if (!(lastDigit === 0 || lastDigit === 8 || lastDigit === 9))
      return { valid: false, reason: 'Century digit out of range (digit 10)', errorCode: 5 };
